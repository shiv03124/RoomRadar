...
const [cities, setCities] = useState([]);
  ...
  const sendOtp = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`https://roomradarbackend-production.up.railway.app/api/otp/send?email=${encodeURIComponent(email)}`, {
        method: 'POST',
      });
  ...
  const verifyOtp = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`https://roomradarbackend-production.up.railway.app/api/otp/verify?email=${encodeURIComponent(email)}&otp=${encodeURIComponent(otp)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const emailRes = await fetch(`https://roomradarbackend-production.up.railway.app/api/users/check-email?email=${encodeURIComponent(email)}`);
      ...
      if (formData.phone) {
        const phoneRes = await fetch(`https://roomradarbackend-production.up.railway.app/api/users/check-phone?phone=${encodeURIComponent(formData.phone)}`);
      ...
      const res = await fetch('https://roomradarbackend-production.up.railway.app/api/users/saveUser', {
        method: 'POST',
        body: formPayload,
      });
  ...
  return (
    <div className="flex min-h-screen w-full ">
      ...
      <div className={`flex flex-col w-full ${margin ? `mt-14` : `mt-0`} ${stage !== 'form' ? 'md:w-1/2' : 'md:w-full'} h-full overflow-auto p-8`}>
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Find Your Perfect Roommate</h1>
        <p className="text-gray-600 mb-8">Create an account to browse through hundreds of rooms</p>
