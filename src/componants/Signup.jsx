...
const [cities, setCities] = useState([]);
  ...
  const fetchCities = async () => {
    try {
      const res = await fetch("http://countriesnow.space/api/v0.1/countries/cities", {
        method: "POST",
  ...
  const sendOtp = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`https://room-radar-kappa.vercel.app/api/otp/send?email=${encodeURIComponent(email)}`, {
        method: 'POST',
      });
  ...
  const verifyOtp = async () => {
    ...
    setError('');
    try {
      const res = await fetch(`https://room-radar-kappa.vercel.app/api/otp/verify?email=${encodeURIComponent(email)}&otp=${encodeURIComponent(otp)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const emailRes = await fetch(`https://room-radar-kappa.vercel.app/api/users/check-email?email=${encodeURIComponent(email)}`);
      ...
      if (formData.phone) {
        const phoneRes = await fetch(`https://room-radar-kappa.vercel.app/api/users/check-phone?phone=${encodeURIComponent(formData.phone)}`);
      ...
      const res = await fetch('https://room-radar-kappa.vercel.app/api/users/saveUser', {
        method: 'POST',
        body: formPayload,
      });
